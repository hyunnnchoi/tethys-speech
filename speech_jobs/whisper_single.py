...
# 분산 학습 스텝 정의를 단일 GPU 학습 스텝으로 변경
...
def train_wav2vec2(model_type="pretraining", num_epochs=1, learning_rate=3e-5):
    """wav2vec2 모델 학습 함수"""
    # 모델 생성
...
========================''')
    ...
    # 모델 학습 실행
    model = train_wav2vec2(model_type="pretraining")
    ...
    try:
        ...
        jct_file = open('/result/' + save_dir_name.strip() + '/single_jct.txt', 'w')
        jct_file.write('%.2f' % (float(jct)))
        jct_file.close()
        model_txt.close()
    except:
        print("JCT 파일 저장 실패, 결과 디렉토리가 없을 수 있습니다.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_batches', type=int, default=40, help='Number of batches per epoch, default is 40')
    parser.add_argument('--batch_size', type=int, default=4, help='Batch size, default is 4')
    args = parser.parse_args()

    # 환경 설정
    # 단일 GPU 환경이므로 분산 학습 관련 설정 제거
    task_type = 'worker'
    task_index = 0
